...
class Visualizer:
    ...
    def plot(self, epoch_str=None, num_epoches=None, epsilon=None, 
            ...
            initial_balance=None, pvs=None, sub_dir=''):
        ...
        for idx, action in zip(x, actions):
            color = 'white'
            if action == 0:
                color = 'r'  # 매수면 빨간색
            elif action == 1:
        ...
        style = ['.r']
        # 정책 신경망의 출력을 빨간색 점으로 그리기
        self.axes[2].plot(x, outvals[:, 0], style)
        ...
        # 에포크 및 탐험 비율
        self.fig.suptitle('Epoch %s/%s (e=%.2f) \n %s' % (epoch_str, num_epoches, epsilon, sub_dir), size=10)
        # 캔버스 레이아웃 조정
        plt.tight_layout()
